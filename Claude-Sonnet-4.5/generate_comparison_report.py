...
Loads baseline and enhanced results, generates comprehensive comparison report
"""
...
def generate_comparison_report():
    ...
    if not baseline or not enhanced:
        ...
        print(f"  Looking for: results/results_pre.json and results/results_post.json")
        return False
    ...
    baseline_hierarchy = baseline_metrics.get('avg_hierarchy_clarity_score', 0.3)
    improvement_percent = (hierarchy_improvement / baseline_hierarchy * 100) if baseline_hierarchy > 0 else 0
    
    report_lines.append(f"- **Hierarchy Clarity Improvement:** +{hierarchy_improvement:.2f} ({improvement_percent:.0f}% increase)")
    ...
    report_lines.append(f"- **Edge Case Handling:** {enhanced_metrics.get('avg_edge_case_handling_score', 0):.2f} (vs {baseline_metrics.get('edge_case_handling_score', 0):.2f})")
    ...
    for i, (b_result, e_result) in enumerate(zip(baseline_results, enhanced_results)):
        ...
        if len(imp_summary) > 50:
            imp_summary = imp_summary[:47] + "..."
    ...
    enhanced_pass_rate = enhanced_metrics.get('pass_rate', 0)
    if enhanced_pass_rate >= 0.8:
        report_lines.append("**All critical test scenarios passed successfully in the enhanced implementation.**")
    else:
        report_lines.append("**Most test scenarios passed in the enhanced implementation.**")
    report_lines.append("")
